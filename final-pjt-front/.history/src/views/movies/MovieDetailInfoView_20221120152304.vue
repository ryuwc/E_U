    ...
    <div class="flex grid grid-cols-3 gap-10">
      <div style="width: 80%" class="col-span-2 md:w-10">
        <div class="relative top-8 left-10 right-10">
          ...
          <p class="text-4xl">{{ movie.thisMovieGenre }}</p>
...
export default {
  ...
  methods: {
    ...
    getMovieGenres() {
      ...
      const keys = Object.keys(this.allGenres);
      for(let i = 0; i < keys.length; i++) {
        const key = keys[i];
        const value = this.allGenres[key];
        for (let i = 0; i < this.movie.genres.length; i++) {
          if (this.movie.genres[i] === Number(key)) {
            this.movieGenres.push(value);
          }
        }
      }
    },
