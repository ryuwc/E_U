    ...
    <div class="flex grid grid-cols-3 gap-10">
      <div style="width: 80%" class="col-span-2 md:w-10">
        <div class="relative top-8 left-10 right-10">
          ...
          <p class="text-4xl">{{ movieGenres[0] }}, {{ movieGenres[1] }}</p>
  ...
  computed: {
    ...
    movieGenres() {
      return this.movie.genre_ids.map((id) => {
        return this.allGenres.find((genre) => genre.id === id).name;
      });
    }
  },
  methods: {
    ...
    getMovieGenres() {
      ...
      for (let i = 0; i < this.allGenres.length; i++) {
        // console.log(this.allGenres[i].id, this.allGenres[i].name);
        for (let j = 0; j < this.movie.genres.length ; j++) {
          if (this.movie.genres[j] === this.allGenres[i].id) {
            // console.log('일치');
            this.movieGenres.push(this.allGenres[i].name);
          }
        }
        // for (let moviegen of this.movie.genre) {
        //   if (moviegen === genre) {
